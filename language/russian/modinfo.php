...
define("_AM_PRINTLIMINATOR_HELP_IS_6" , "Для создания файла filecheck.txt возьмите файл <a href='../docs/md5.rar'>docs/md5.rar</a>, распакуйте его в корень сайта и запустите. В корне будет создан файл filecheck.txt. Проверьте его и удалите в начале строк путь до корня сайта. Затем скопируйте в /modules/printliminator/admin с заменой старого файла.<br /><strong style='color:red;'>Внимание:</strong> после использования скрипта не забудьте удалить скрипт md5.php и файл filecheck.txt с корневой дирректории Вашего сайта!");
